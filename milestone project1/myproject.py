...
class BoardGame:
    ...
    def win_check(self, board, mark):
        return (
            ...
            or (  # across the top
                board[3] == mark and board[4] == mark and board[5] == mark
            )
            ...
            or (board[8] == mark and board[4] == mark and board[0] == mark)  # diagonal
        )  # diagonal

    def play(self, name, board, position, maker):
        if (
            (maker == "X" or maker == "O")
            and len(board) > int(position) >= 0
            and board[int(position)] not in ["X", "O"]
        ):
